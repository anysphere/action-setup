...
async function handler(input, opts) {
    ...
    if (opts.workspace) {
        ...
        if (!opts.linkWorkspacePackages && !opts.saveWorkspaceProtocol) {
            if (opts.rawLocalConfig['save-workspace-protocol'] === false) {
                throw new error_1.default('BAD_OPTIONS', common_tags_1.oneLine `This workspace has link-workspace-packages turned off,
    ...
    if (opts.recursive && opts.allProjects && opts.selectedProjectsGraph && opts.workspaceDir) {
        await recursive_1.default(opts.allProjects, input, {
            ...opts,
    ...
    if (opts.workspaceDir) {
        allProjects = allProjects !== null && allProjects !== void 0 ? allProjects : await find_workspace_packages_1.default(opts.workspaceDir, opts);
    ...
    const updateMatch = opts.update && input.length ? recursive_1.createMatcher(input) : null;
    if (updateMatch) {
        input = recursive_1.matchDependencies(updateMatch, manifest, includeDirect);
        if (!input.length) {
    ...
    if (opts.update && opts.latest) {
        if (!input || !input.length) {
            input = cli_utils_1.updateToLatestSpecsFromManifest(manifest, includeDirect);
        }
        else {
    ...
    if (opts.workspace) {
        if (!input || !input.length) {
            input = updateWorkspaceDependencies_1.updateToWorkspacePackagesFromManifest(manifest, includeDirect, workspacePackages);
        }
        else {
            input = updateWorkspaceDependencies_1.createWorkspaceSpecs(input, workspacePackages);
        }
    }
    if (!input || !input.length) {
