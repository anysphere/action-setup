...
async function recursive(allProjects, input, opts) {
    var _a;
    ...
    const rebuild = (input.length === 0
        ? implementation_1.rebuild
        : (importers, opts) => implementation_1.rebuildPkgs(importers, input, opts) // tslint:disable-line
    );
    if (opts.lockfileDir) {
