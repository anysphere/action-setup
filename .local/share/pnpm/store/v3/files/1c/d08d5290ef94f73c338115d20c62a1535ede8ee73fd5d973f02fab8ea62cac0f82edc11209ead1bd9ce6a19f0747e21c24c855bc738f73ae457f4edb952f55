...
async function run(inputArgv) {
    var _a;
    const { argv, cliArgs, cliConf, cmd, subCmd, isKnownCommand, unknownOptions, workspaceDir } = await parseCliArgs_1.default(inputArgv);
    if (!isKnownCommand) {
    ...
    if (cliConf['recursive']) {
        ...
        const allProjects = await find_workspace_packages_1.default(wsDir, config);
    ...
    await new Promise((resolve, reject) => {
        setTimeout(() => {
            ...
            try {
                const result = cmd_1.default[cmd || 'help'](cliArgs, 
