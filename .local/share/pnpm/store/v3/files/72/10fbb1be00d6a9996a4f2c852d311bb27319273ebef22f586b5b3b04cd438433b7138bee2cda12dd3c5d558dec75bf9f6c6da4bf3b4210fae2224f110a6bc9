...
async function mutateModules(projects, maybeOpts) {
    ...
    async function _install() {
        if (!opts.lockfileOnly &&
            ...
            (opts.frozenLockfile ||
                ...
                opts.preferFrozenLockfile &&
                    ...
                    await p_every_1.default(ctx.projects, async (project) => !hasLocalTarballDepsInRoot(ctx.wantedLockfile, project.id) &&
                        lockfile_utils_1.satisfiesPackageManifest(ctx.wantedLockfile, project.manifest, project.id) &&
        ...
        for (const project of ctx.projects) {
            switch (project.mutation) {
                ...
                case 'unlink': {
                    ...
                    const allDeps = utils_1.getAllDependenciesFromPackage(project.manifest);
        ...
        async function installSome(project) {
            const currentPrefs = opts.ignoreCurrentPrefs ? {} : utils_1.getAllDependenciesFromPackage(project.manifest);
...
async function linkedPackagesAreUpToDate(manifest, projectSnapshot, prefix, workspacePackages) {
    var _a, _b;
    ...
    for (const depField of types_1.DEPENDENCIES_FIELDS) {
        ...
        for (const depName of depNames) {
            ...
            const dir = isLinked
                ...
                : (_b = (_a = workspacePackages === null || workspacePackages === void 0 ? void 0 : workspacePackages[depName]) === null || _a === void 0 ? void 0 : _a[importerDeps[depName]]) === null || _b === void 0 ? void 0 : _b.dir;
            if (!dir)
                continue;
            const linkedPkg = workspacePackagesByDirectory[dir] || await utils_1.safeReadPackageFromDir(dir);
...
async function installInContext(projects, ctx, opts) {
    ...
    const projectsToLink = await Promise.all(projectsToResolve.map(async (project, index) => {
        ...
        const topParents = project.manifest
            ? await getTopParents(R.difference(Object.keys(utils_1.getAllDependenciesFromPackage(project.manifest)), resolvedImporter.directDependencies
