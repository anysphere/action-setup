...
async function recursive(allProjects, input, opts, cmdFullName) {
    ...
    let pkgPaths = chunks.length === 0
        ? chunks[0]
        : Object.keys(opts.selectedProjectsGraph).sort();
    const limitInstallation = p_limit_1.default((_b = opts.workspaceConcurrency) !== null && _b !== void 0 ? _b : 4);
...
function matchDependencies(match, manifest, include) {
    const deps = Object.keys(utils_1.filterDependenciesByType(manifest, include));
    const matchedDeps = [];
